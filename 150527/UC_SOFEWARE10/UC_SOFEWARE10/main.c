...
/*********************************
    15782,15956,16125,16288,16448,16605,16757,16907,17029,17148,17266,
    17380,17493,17593,17692,17789,17884,17978,18060,18141,18221,18299,18377,18443,18509,18573,18637,18700,18756,18811,18865,18919,
    18972,19022,19072,19121,19170,19219,19255,19291,19326,19362,19397,19430,19462,19495,19527,19559,19586,19613,19641,19668,19695,
    19719,19742,19766,19790,19814,19832,19851,19870,19888,19907,19926,19945,19963,19982,20001,
*******273.5 K *****/																			  
...
unsigned int Temp_Count,P_count;
		...
		while(TEMP)
		   { ; P_count = P_count + 1 ;}	  
		    ...
		    TEMP = 0 ;
	     if(P_count > 0x7c80){Temp_Count = 0;}	 //7010
	   	 else{ Temp_Count = ( 0x7c80 - P_count)/19.4; //9.936 ;1760 2329 
	 	   if(Temp_Count > 121)
     ...
     { 
      ...
      if (Soft_Timer[TEMP_TIME] == 0)
		  ...
		  Soft_Timer[TEMP_TIME] = 4;
	       ...
	       if(n_code < 4){n_code++;}
	      else{ Temp_m = (Data_Value[3]/4+Data_Value[2]/4+Data_Value[1]/4+Data_Value[0]/4) ;
				  n_code = 0 ;
		...
		if (Soft_Timer[ADC_READ_TIME] == 0) 
		  {
			 ...
			 if(t_code < 13)
              ...
              {	
			    Temp_sp = (Temp_m*14)/10;   //35 1.54 /0k
		        TT_Value = Check_Type_Offset(Code_Value);
             ...
             { 	NP_COD = 1;
			    Temp_sp = (Temp_m*3)/10;//Temp_offset/12;
				TT_Value = Check_Type_Offset(Code_Value);
	    ...
	    if (Soft_Timer[DISPLAY_SEND_TIME] == 0)
	      {
		     Soft_Timer[DISPLAY_SEND_TIME] = 32;	
			   LED_Disp(1,TC_Value);
//			   LED_Disp(1,Temp_m);
...
unsigned int Check_Type_Offset(unsigned int Code_Value)
{
     ...
     if (NG_COD == 0)
		   ...
		   if(Code_Value >= TC_Zero_offset)	   
		     {
	          if((Code_Value - TC_Zero_offset) >= Temp_sp)
	             { Count_Value = (Code_Value - TC_Zero_offset) - Temp_sp ;}
	           else
			     {Count_Value = (Temp_sp + TC_Zero_offset)- Code_Value; AV_COD = 1 ; }
	          }
			 else
			  {   
		       if((TC_Zero_offset - Code_Value) >= Temp_sp)
			    {Count_Value = TC_Zero_offset - (Code_Value + Temp_sp);AV_COD = 1 ;}
	           else
	            { Count_Value =  (Temp_sp + TC_Zero_offset) - Code_Value; AV_COD = 1 ; }
		      }
		   }
          else	  // AV_COD = 1	 NG_COD = 0
	       {
		     if(Code_Value >= TC_Zero_offset)
			   {
			    if((Code_Value - TC_Zero_offset) >= Temp_sp)
			      ...
			      { Code_Value =  Temp_sp + ( Code_Value + TC_Zero_offset);}
			   }
		     else
			   {	  // AV_COD = 1	 NG_COD = 0
			    if((TC_Zero_offset- Code_Value ) >= Temp_sp)
      ...
      else
	   {
	     if(AV_COD == 0)  // AV_COD = 0	 NG_COD = 1
		   {
			 Count_Value = Code_Value + TC_Zero_offset ;
		    if(Count_Value >= Temp_sp)
		     { Count_Value =  Count_Value - Temp_sp ;}
	        else
	         {Count_Value = Temp_sp  - Count_Value; AV_COD = 1; }
	       }
		     ...
		     if(Code_Value >= TC_Zero_offset)
			   {
			    ...
			    if(Count_Value >= Temp_sp)
			      ...
			      { Count_Value = Temp_sp + Count_Value;}
			   }
		     else
			   {
				 Count_Value = TC_Zero_offset - Code_Value;
			    if( Count_Value >= Temp_sp)
		          { Count_Value =  Count_Value - Temp_sp ; AV_COD = 0;}
			    else
			      { Count_Value = Temp_sp -Count_Value; AV_COD = 1;}
			   }														   
		   }
   ...
   if(ADD_COD == 1 ) 
	 { Count_Value = Count_Value + (Count_Value/TC_Span_Value); }	  //TC_Span_Value
   else
	 { Count_Value = Count_Value - (Count_Value/TC_Span_Value);}
		        ...
		        if (Itemm == MODE_CAL)			//tc,rtd,mV,mA
		          {
	   	         ...
	   	         else if (Itemm == TYPE_CAL)
		            {
					    if (Work_Type != 0)
						  {	Itemm = Itemm + 1;}
							  ...
							  if (TC_Type >= 6)
								...
								if(confrom_key == 1)
							     {
								   EEPROM_write_byte(Type_Select,TC_Type);  //K,J,T,E,R,B,S
								   times ++; SETUP_LED_Disp(22,0);delay(2500);
								 }
						     }
					 }		
				else if (Itemm == TC_ZERO_CAL)
					  {
					   if (Work_Type != 0){	Itemm = Itemm + 1;}
							   ...
							   SETUP_LED_Disp(Itemm, Data_Value[14]);
							   if(confrom_key == 1)
							    {
								  Temp_Flag = 3 ;
								  Temp_offset = Temp_m;
								 if ((Temp_m*24)/10 >= Data_Value[14])
								   {
								     if( AV_COD == 1)
								       { TC_Zero_offset = (Temp_m*24)/10 + Data_Value[14]; NG_COD = 1;use_flag = use_flag|0x01;}
								     else{ TC_Zero_offset = (Temp_m*24)/10 - Data_Value[14]; NG_COD = 1;use_flag = use_flag|0x01;}
								   }
								 else
								   {  
								   	 if( AV_COD == 1)
								       { TC_Zero_offset = Data_Value[14] + (Temp_m*24)/10 ;  NG_COD = 1;use_flag = use_flag|0x01;}
								     else {TC_Zero_offset = Data_Value[14] - (Temp_m*24)/10;  NG_COD = 0;use_flag = use_flag&0xfe;}
								   }
	              ...
	              else if (Itemm == TC_SPAN_CAL)
		            {
					  ...
					  if(confrom_key == 1)
					    { 
						 if( NG_COD == 1)
						    {TC_Span_offset = Data_Value[14] + TC_Zero_offset;}
					    else {TC_Span_offset = Data_Value[14] - TC_Zero_offset;}
						        EEPROM_write_word(TC_Span,TC_Span_offset);  

					     if(TC_Span_offset >= 0xc8ca )
						   { TC_Span_Value = (TC_Span_offset + TC_Zero_offset) /(TC_Span_offset - 0xc8ca);use_flag = use_flag|0x10;}
						 else
						   { TC_Span_Value = (TC_Span_offset + TC_Zero_offset) /(0xc8ca - TC_Span_offset);use_flag = use_flag&0xef;}
...
void EEPROM_WriteDefault(void)
{
	...
	EEPROM_write_byte(Week_Date,0x04);
