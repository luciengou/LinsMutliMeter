...
unsigned int Temp_Count,P_count;
		...
		while(TEMP)
		   { ; P_count = P_count + 1 ;}	  
		    TEMP = 0 ;
	     if(P_count > 0x7a10){Temp_Count = 0;}	 //7010
	   	 else{ Temp_Count = ( 0x7a10 - P_count)/19.4; //9.936 ;1760 2329 
	 	   if(Temp_Count > 121)
		...
		if (Soft_Timer[ADC_READ_TIME] == 0) 
		  {
             ...
             { 	NP_COD = 1;
			    Temp_sp = (Temp_m*5)/10;//Temp_offset/12;
				TT_Value = Check_Type_Offset(Code_Value);
...
unsigned int Check_Type_Offset(unsigned int Code_Value)
{
   ...
   if(ADD_COD == 1 ) 
	 { Count_Value = Count_Value - (Count_Value/81); }	  //TC_Span_Value
   else
	 { Count_Value = Count_Value + (Count_Value/81);}
					  ...
					  if(confrom_key == 1)
					    { 
					     ...
					     if(TC_Span_offset >= 0xc8ca )
						   { TC_Span_Value = TC_Span_offset /(TC_Span_offset - 0xc8ca);use_flag = use_flag|0x10;}
						 else
						   { TC_Span_Value = TC_Span_offset /(0xc8ca - TC_Span_offset);use_flag = use_flag&0xef;}
